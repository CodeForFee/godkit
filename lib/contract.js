...
const { looksLikeModelId, why } = require('./identity')
...
function checkTask(agentDir, file) {
  ...
  } else if (started && !looksLikeModelId(meta.owner)) {
    // An owner is only useful if the next agent can tell what wrote it. See lib/identity.js.
    out.push(finding('task', label, 'no-identity', 'owner is ' + why(meta.owner)))
  }
...
function checkLog(agentDir, file) {
  ...
  if (!looksLikeModelId(meta.agent)) {
    out.push(finding('log', label, 'no-identity', 'agent is ' + why(meta.agent)))
  }
