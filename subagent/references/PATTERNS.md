...
Reference for the **subagent** skill. Load this only when you are designing an orchestration
mechanism — a delegation seam, a retry policy, a shared-state contract. Ordinary work does not
need it.

These are distilled from a production agent harness (`deepseek-harness`). Each one is a class of
defect that actually shipped somewhere, stated as the rule that prevents it.
...
and subagent providers through the same seam: several coexist, each declares what it supports, the
